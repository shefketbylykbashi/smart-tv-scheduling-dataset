...
def time_weight(minute: int) -> float:
    """
    Prime time boost. You can tune this.
    - morning: 06:00-10:00 moderate
    - daytime: 10:00-17:00 normal
    - evening/prime: 17:00-23:00 high
    """
    if 360 <= minute < 600:
        return 1.08
    if 600 <= minute < 1020:
...
def compute_score(channel_key: str, title: str, desc: str, genre: str, start_min: int, duration: int) -> int:
    ...
    noise = (stable_int(f"{channel_key}|{title}|{start_min}") % 13) - 6  # [-6..+6]
...
def stream_epg(url_or_path: str) -> Iterable[Tuple[str, dict]]:
    ...
    if url_or_path.startswith("http://") or url_or_path.startswith("https://"):
        fh = urllib.request.urlopen(url_or_path)
    else:
